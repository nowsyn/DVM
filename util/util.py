...
def tensor2im(input_image, imtype=np.uint8, scaling=True, use_rgb=False):
    ...
    if not isinstance(input_image, np.ndarray):
        ...
        if image_numpy.shape[0] == 1:  # matte to RGB
    ...
    # TODO: remove it if the data loading is changed.
    if not use_rgb:
...
def save_image_np(image_numpy, image_path):
    """Save a numpy image to the disk
