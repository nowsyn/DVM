...
class BaseModel(ABC):
    ...
    def update_learning_rate(self, total_iters=0, lr=None):
        """Update learning rates for all the networks; called at the end of every epoch (iteration)"""
        ...
        if lr is not None:
            self.optimizers[0].param_groups[0]['lr'] = max(self.opt.min_lr, lr)
        else:
            self.optimizers[0].param_groups[0]['lr'] = max(self.opt.min_lr, cur_lr)
        cur_lr = self.optimizers[0].param_groups[0]['lr']
        if self.opt.lr_policy != 'cyclic':
            print('learning rate = %.7f' % cur_lr)
        return lr
    ...
    def get_latest_epoch(self):
        latest_epoch = 1
        for pth in glob.glob(self.save_dir+"/*.pth"):
            try:
                epoch = int(os.path.basename(pth).split('_')[0])
    ...
    def load_networks(self, epoch, strict=True):
