...
def tfrecord_index_loader(index_path):
    index = np.loadtxt(index_path, dtype=np.int64)[:, 0]
    return range(len(index))


def tfrecord_iterator(data_path, index_path=None, shard=None, chunk=None):
    ...
    datum_bytes = bytearray(4*1024*1024)
    ...
    def read_records(start_offset=None, end_offset=None):
        ...
        while file.tell() < end_offset:
            ...
            if length <= len(datum_bytes):
                ...
                datum_bytes_view = memoryview(datum_bytes.zfill(int(length * 2)))[:length]
...
def tfrecord_loader(data_path, index_path, description, shard=None, chunk=None):
    ...
    """
    record_iterator = tfrecord_iterator(data_path, index_path, shard, chunk)
